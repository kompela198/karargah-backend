...
app = FastAPI(title="Karargah Backend v1.2 - Bulut Mimarisi")
...
def init_db():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS servers (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL, owner TEXT NOT NULL, icon_url TEXT DEFAULT '')""")
    try: cursor.execute("ALTER TABLE servers ADD COLUMN icon_url TEXT DEFAULT ''")
    except: pass
    cursor.execute("""CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, server_name TEXT NOT NULL, channel_name TEXT NOT NULL, sender TEXT NOT NULL, text TEXT NOT NULL, time_str TEXT NOT NULL)""")
...
def check_prime(username: str):
    ...
    conn.close()
    is_prime = result[0] if result else 0
...
# BULUT İÇİN GÜNCELLENEN AKILLI GÖRSEL YÜKLEME (127.0.0.1 KALDIRILDI)
@app.post("/api/upload")
...
def get_servers():
    conn = sqlite3.connect("karargah.db")
    cursor = conn.cursor()
    cursor.execute("SELECT name, owner, icon_url FROM servers")
...
class JoinServerData(BaseModel):
    username: str
    invite_code: str
...
def join_server(data: JoinServerData):
    ...
    result = cursor.fetchone()
    conn.close()
    if result: return {"status": "success", "server_name": result[0]}
